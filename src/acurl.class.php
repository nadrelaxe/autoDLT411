 ...
 * File: acurl.class.php5
 * Author: Sylvain 'Absynthe' Rabot <sylvain@abstraction.fr>
 * Website: http://absynthe.is.free.fr/acurl/
 * Version: alpha 1
 * Date: 05/06/2007
 * License: LGPL
 */
 ...
 * $acurl = new acurl();
 *
 * $acurl->set_option('url', 'http://absynthe.is.free.fr/');
 * $acurl->set_option('port', 80);
 * $acurl->set_option('login', 'Absynthe');
 * $acurl->set_option('password', 'mypassword');
 * $acurl->set_option('headers', true);
 * $acurl->set_option('cookie', '/path/to/my/cookie.txt');
 * $acurl->set_option('user_agent', 'Mozilla/5.0 (Windows; U; Windows NT 5.1; fr; rv:1.8.1.4) Gecko/20070515 Firefox/2.0.0.4');
 *
 * $answer = $acurl->ping();
 * $answer = $acurl->http_request();
 * $answer = $acurl->http_auth_request();
 * $answer = $acurl->http_post_request('var=foo&var1=fooo');
 * $answer = $acurl->http_post_request(array('var' => 'foo', 'var1' => 'fooo'));
 * $answer = $acurl->http_post_auth_request('var=foo&var1=fooo');
 * $answer = $acurl->http_post_auth_request(array('var' => 'foo', 'var1' => 'fooo'));
 *
 * $answer = $acurl->ftp_upload('/path/to/my/file.txt', 'ftp.abstraction.fr/path/to/file.txt', 21, 'Absynthe', 'mypassword');
 * $answer = $acurl->ftp_download('/path/to/my/file.txt', 'ftp.abstraction.fr/path/to/file.txt', 21, 'Absynthe', 'mypassword');
 *
 * </code>
 */
